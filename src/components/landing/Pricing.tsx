...
const Pricing = () => {
  return <section id="precos" className="py-24 relative overflow-hidden">
      {/* Background */}
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {plans.map((plan, index) => <div key={index} className={`relative p-6 md:p-8 rounded-3xl border card-shadow transition-transform hover:scale-105 ${plan.popular ? "gradient-card border-connexia-magenta/30 animate-pulse-glow" : "bg-card/50 border-border/50"}`}>
              {/* Popular Badge */}
              {plan.popular && <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                  <div className="flex items-center gap-2 py-2 rounded-full gradient-primary px-[17px]">
              ...
              <div className="space-y-4 mb-8">
                <div className="flex items-start gap-3">
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${plan.popular ? "gradient-primary" : "bg-connexia-cyan/20"}`}>
